...
with col3:
    ...
    for key, value in target["Gender"].items():
        ...
        eligible_pop = (current_us["Gender"].get(key, 0) / 100) * total_disease_pop / 100) * total_disease_pop
        fail_rate = 1 - (st.session_state.get(f"sf_gender_{key}", 100) / 100)
        screened_needed = target_n / (1 - fail_rate)
        eligible_pop = (current_us["Race"].get(key, 0) / 100) * total_disease_pop / 100) * total_disease_pop
