...
with col3:
    ...
    for key, value in target["Gender"].items():
        ...
        gender_results.append((key, screened_needed, screen_percent))

    for key, screened_needed, screen_percent in sorted(gender_results, key=lambda x: -x[2]):
    ...
    for key, value in target["Race"].items():
        ...
        screened_needed = target_n / (1 - fail_rate)
        screen_percent = (screened_needed / eligible_pop) * 100 if eligible_pop > 0 else 0
        race_results.append((key, screened_needed, screen_percent))

    for key, screened_needed, screen_percent in sorted(race_results, key=lambda x: -x[2]):
        st.markdown(f"{key}: {int(screened_needed):,} (**{screen_percent:.3f}%**) ")
